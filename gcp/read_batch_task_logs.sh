...
#
# Examples:
#   ./gcp/read_batch_task_logs.sh escher-exp3-checkpoint-20260523-120000
#   ./gcp/read_batch_task_logs.sh escher-exp3-checkpoint-20260523-120000 ERROR Traceback Killed
#
...
for line_number, line in enumerate(log_path.read_text(encoding="utf-8", errors="replace").splitlines(), start=1):
    for candidate in sorted(set(job_name_pattern.findall(line))):
        ...
        if target_job.startswith(f"{candidate}-"):
            ...
            # outputs/cloud/escher-exp1-baseline while the Batch job name has a
